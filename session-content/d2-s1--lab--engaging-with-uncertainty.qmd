---
title: "Engaging With Uncertainty"
format:
  html:
    include-before-body: images/cc-data-banner.html
    toc: true
    standalone: true
    theme: cerulean
    mainfont: Gilroy
...
LoadPackages(tidyr, dplyr, purrr, ggplot2, gt)
...
( things take a bit more of a technical / statsy / jargony twist --- maybe )
:::
...
At several points during the activity you will be asked to take a record of thoughts, ideas and questions surfaced by your group, as well as some of the values output by the app.
Please write anything that occurs down because some of these will be referred to at later points int the activity.
You can choose whether to keep your own notes or to nominate a scribe for the group.

## Scenario

Consider a situation where a business has an average weekly sales value of exactly £100.
...
## **1. Formulate business relevant questions**

With your colleague(s) discuss what more information you would like to know before agreeing or disagreeing with the Boss.
...
![Sample average (observed) vs expected average](images/plot--improved-sales.png){#fig-exp-vs-sample alt="Plot showing a horizontal line labelled \"Expected\" at y = £100, and an \"Observed\" value of £109.66 " fig-align="left" width="40%"}
...
For our purposes, we will assume a change of at least **1 percentage point** to be an outcome that has real importance for our business.
...
You can create your table on paper or using a desktop app.
Record your value in your table.
Please keep an easily referenced

Now... unbeknownst to the Boss we have been granted superhero like powers that allow us to explore different realities.
We have the ability to simulate what the observed value might be like in some of these other realities, by pressing the ![](images/btn--new-sample){width="108" alt="\"New sample\""} button.
...
![](images/input--n-sales-5.png){#fig-n-sales-input display="inline" width="300" alt="sample size input, set to n = 5"}

The first control we will look at on the left is labeled "Number of sales".
...
![](images/input--cb-show-sales.png){width="300" alt="two checkboxes labeled \"Margin of error\" and \"Sales\". The bottom box, \"Sales\" has been checked"}
...
Press the ![](images/btn--new-sample.png){width="108" alt="\"New sample\""} button several more times.
...
Remember, the Boss has no idea about any of these alternative realities.
How might you convey these insights to the Boss?
:::

In your table, create a new column with the header `n = 20` .
Now change the "Number of sales" input to 20, and try several new samples (press ![](images/btn--new-sample.png){width="107" alt="\"New sample\""}).
...
Try a new column `n = 100` and compare those values.
:::
...
If you carried out the last instruction in the box above, you show have number of sales set to 100.
The input below that is called "Spread of values".
Change this input to 50, hit ![](images/btn--new-sample.png){width="107" alt="\"New sample\""} a few more times.
Hmm, things just got unpredictable again, right?

::: {.callout-tip icon="false"}
#### **7. Discuss**

What does this "Spread of values" input represent---in a real-world/business context?
...
Which business should be able to make the most accurate projections for the future?
...
This what the mean does.
while the estimate may jump about wildly when the sample size (number of sales) is small, it does so around the actual true mean.
...
To get a better understanding of how the "spread" relates to the precision, check the "Show margin of error" box (right, bottom).
...
![](images/vb--ci-unknown.png){width="250" alt="Value box, showing a confidence interval of unknown value. The colour of the title bar is yellow, indicating the value is unknown, or does not cover the expected value of 100."}

![](images/input--cb-show-moe-sales.png){width="250" alt="Two checkboxes labeled \"Margin of error\" and \"Sales\". Both boxes have been checked."}

![](images/vb--ci-known-sig.png%20alt='Value%20box%20displaying%20a%20margin%20of%20error%20of%20102.5-118.2.%20The%20title%20bar%20of%20the%20box%20is%20green%20indicating%20that%20the%20margin%20of%20error%20does%20not%20cover%20the%20expected%20value%20of%20100.')
:::
...
Also have think about what those combinations might mean in a real-world context.
...
The language for dealing with these is a bit statsy and confusing, so we'll work try and set some expectations first which we will then try and test to confirm or refute.
...
![](images/input--rb-ci-95.png){width="250" alt="Radio button allowing choices for the margin of error between 90%, 95% and 99%. Displays 95% as selected"}
...
2.  We don't know what that is, that is why we are guessing.
...
Remember our question is to investigate whether our marketing campaign has caused a shift in out customer base.
...
::: columns
::: column
![](images/vb--p-998.png){width="210" alt="Value box showing a p-value of 0.998. The title bar is blue indicating the results is non-significant."}

![](images/vb--pval-sig.png){width="213" alt="Value box showing a p-value of 0.013. The title bar is green indicating the results is higher than the expected result to a meaningful degree."}
:::

::: column
This magical number is displayed under the plot, to the left of the CI box.
...
This is a metric that asks if the customer base **had not** changed, how unusual would the data obtained from the new customer base be?
:::
:::

The more weird the data is, the more different the new average will be from the old average, and the smaller this magic number is.
...
![](images/plot--distrs-sig-neg.png){width="30%" alt'Two plot stacked verically.
Top: an "s" shaped curve with the y axis ranging between 0 and 1.
Bottom: a bell shaped curve with the y axis ranging from 0 to 0.4.
The x axes range from -4 to 4.
There is a point on each curve at x = -3, and is very close to 0 on the y axis.
These points are shown to be further left than the significance threshold, shown at -2'}
:::
...
In the example there are enough rows to sample 6 observation -- you may want a few more.
...
The top part is labeled **Below** and should be used for recording sales values that come in under the expected value of the old customer base.
...
Hit ![](images/btn--new-sample.png){width="107" alt="\"New sample\""} enough times to fill your table.
...
Hit the ![](images/btn--new-sample.png){width="108" alt="\"New sample\""} button a few times, use what we have already learned today to try and get a p-value below the threshold.
...
4.  The Boss wants to try a new marketing campaign.
    -   Hit the ![](images/btn--new-campaign.png){width="122" height="40" alt='New campaign"} button at the top of the menu bar to target a different customer base, and start again!
