...
-   [Combination](images/d2-s4--growth-plot.png){.external target="_blank"}
...
#| eval: false
#| include: false
plt_imgs <- grep('btn|input|vb|icon|plot|banner|placeholder|full-image', dir('images/'), value = TRUE, invert = TRUE)

purrr::map_chr(plt_imgs, ~{
  sprintf('[%s](%s){.external target="_blank"}\n', basename(.), .)
})
```
:::
