...
![](../images/egregious-plot_mil-spend.png){width='65%'}

Some might use such a figure to push a particular agenda. "Clearly China is a massive threat to global security!"
...
![](../images/egregious-plot_fixed_mil-spend.png){width='60%'}
...
1. All countries should be measured on the left axis.
1. __Except__ the U.S., which has its own. different, scale on the right.
1. The U.S. specific scale is not zeroed - it begins at $400,000,000,000!
1. Particularly unhelpful (disingenuous?) is that China's line rises to its maximum right next to the figure $950 billion.
1. When measured on the left axis, or having the same scale on both axes, we see that China does not even make it onto the right hand scale of the original plot.
...
![](../images/egregious-plot_height.png){width='60%'}

Also an example of inappropriate use of icons - we are forced to compare the __areas__ of the figures, so Indonesians appear tiny and the Dutch appear giant.
...
![](../images/egregious-plot_fixed_height.jpg){width='60%'}
...
![](../images/egregious-plot_fixed-minimal_height.png){width='60%'}

Here, we lose the silhouettes, which might be conidered by some as "chart junk".
...
1. The use of human shaped images that are scaled in proportion to the non-zeroed scale mean we compare not only the height of the figures, but also their _areas_.
1. The second plot appeared on the same site some time after the original - maybe someone had a word with someone?
1. The third plot suggests a more minimal way of showing the same data focusing only on height
...
> Nonsense! Under our leadership interest rates have remained remakably stable!
...
1. The __truncated__ axis is a problem
    - because we compare areas of the bars.
1. The __non-truncated__ axis is a problem
    - because the differences now totally disappear.
1. The final plot retains the, often abused, truncated axis
